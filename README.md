...
- 支持 CSV / Excel
- 默认使用最新订单日期作为分析基准
- 支持日期范围筛选，并作为产品线和 SKU 筛选的上游条件
- 未选择 SKU 时分析全部 SKU
- 支持产品线筛选，适合同一订单文件包含多个产品线的场景
- SKU 分析以商品 ID 为核心统计维度，商品名只作为辅助说明
- SKU 分析支持查看已筛选商品 ID 合计订单、收入和总盘占比
- AB 测试支持按用户 ID 尾数分 A/B 方案，查看订单量、订单占比、订单金额和金额占比
- 用户分析支持支付间隔分桶，可自定义天数边界
